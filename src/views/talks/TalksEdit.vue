  ...
  <div class="talks-edit mw-basic-layout">
    <div class="mw-content">
      <app-title
        ...
        :title="
          isUpdatingMode
            ? $t('TALKS_EDIT.LABEL.UPDATE')
            : $t('TALKS_EDIT.LABEL.CREATE')
        "
      >
        <template slot="back">
        ...
        <md-card>
          <md-card-content>
            <md-field :class="$v.talk.title.$invalid ? 'md-invalid' : ''">
              ...
              <md-input :disabled="isSaving" v-model.trim="talk.title">
              </md-input>
              <span class="md-error" v-if="!$v.talk.title.required">
                {{ $t('TALKS_EDIT.ERRORS.REQUIRED') }}
              </span>
            </md-field>
            <md-field :class="$v.talk.abstract.$invalid ? 'md-invalid' : ''">
              <label>{{ $t('TALK.ABSTRACT') }}</label>
              <md-textarea :disabled="isSaving" v-model.trim="talk.abstract">
              </md-textarea>
              <span class="md-error" v-if="!$v.talk.abstract.required">
                {{ $t('TALKS_EDIT.ERRORS.REQUIRED') }}
              </span>
            </md-field>
            <md-field :class="$v.talk.level.$invalid ? 'md-invalid' : ''">
              <label>{{ $t('TALK.LEVEL') }}</label>
              <md-select :disabled="isSaving" v-model="talk.level">
              ...
              <span class="md-error" v-if="!$v.talk.level.required">
                {{ $t('TALKS_EDIT.ERRORS.REQUIRED') }}
              </span>
            </md-field>
            <md-field :class="$v.talk.room.$invalid ? 'md-invalid' : ''">
              <label>{{ $t('TALK.ROOM') }}</label>
              <md-select :disabled="isSaving" v-model="talk.room">
              ...
              <span class="md-error" v-if="!$v.talk.room.required">
                {{ $t('TALKS_EDIT.ERRORS.REQUIRED') }}
              </span>
            </md-field>
            <md-field :class="$v.talk.hour.$invalid ? 'md-invalid' : ''">
              <app-datetime mode="time" v-model="talk.hour"> </app-datetime>
              <span class="md-error" v-if="!$v.talk.hour.required">
                {{ $t('TALKS_EDIT.ERRORS.REQUIRED') }}
              </span>
            </md-field>
            <md-autocomplete
          ...
          <md-card-actions>
            ...
            <md-button
              :disabled="$v.talk.$invalid || isSaving"
...
export default {
    ...
    Promise.all(promises)
      .then(datas =>
        next(vm => {
          vm.edition = datas[0];
          vm.speakers = datas[1];
          vm.speakersNames = Object.values(datas[1])
            .map(speaker => speaker.displayName)
            .filter(speaker => speaker !== null);

          if (datas.length === 3) {
            vm.talk = datas[2];
            vm.selectedSpeakersNames = Object.keys(vm.talk.speakers).map(
              speakerId => vm.speakers[speakerId].displayName
            );
  ...
  methods: {
    selectSpeaker() {
      ...
      const speakerId = Object.keys(this.speakers).find(
        speakerId =>
          this.speakers[speakerId].displayName === this.selectedSpeakersName
      );
    ...
    deselectSpeaker(text, index) {
      const speakerId = Object.keys(this.speakers).find(
        speakerId => this.speakers[speakerId].displayName === text
      );

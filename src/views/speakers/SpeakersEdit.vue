  ...
  <div class="speakers-edit mw-basic-layout">
    <div class="mw-content">
      <app-title
        ...
        :title="
          isUpdatingMode
            ? $t('SPEAKERS_EDIT.LABEL.UPDATE')
            : $t('SPEAKERS_EDIT.LABEL.CREATE')
        "
      >
        <template slot="back">
      ...
      <form @submit.prevent="save" novalidate>
        <md-card>
          <md-card-content>
            ...
            >
              <label>
                <span v-if="isLoadingConferenceHall">{{
                  $t('SPEAKERS_EDIT.CONFERENCE_HALL.LOADING')
                }}</span>
              ...
              <span class="md-error" v-if="!$v.speaker.displayName.required">
                {{ $t('SPEAKERS_EDIT.ERRORS.REQUIRED') }}
              </span>
            </md-autocomplete>
            <md-field>
              <label>{{ $t('SPEAKER.ROLE') }}</label>
              <md-input :disabled="isSaving" v-model.trim="speaker.role">
              </md-input>
            </md-field>
            <div class="field-img">
              <div class="img">
                <img :src="speaker.photoURL" alt="" v-if="speaker.photoURL" />
                <md-icon v-else>account_circle</md-icon>
              </div>
              <div class="field">
                <md-field
                  :class="$v.speaker.photoURL.$invalid ? 'md-invalid' : ''"
                >
                  ...
                  <span class="md-error" v-if="!$v.speaker.photoURL.required">
                    {{ $t('SPEAKERS_EDIT.ERRORS.REQUIRED') }}
                  </span>
                </md-field>
              </div>
            ...
            <md-field :class="$v.speaker.company.$invalid ? 'md-invalid' : ''">
              ...
              <md-input :disabled="isSaving" v-model.trim="speaker.company">
              </md-input>
              <span class="md-error" v-if="!$v.speaker.company.required">
                {{ $t('SPEAKERS_EDIT.ERRORS.REQUIRED') }}
              </span>
            </md-field>
            <md-field :class="$v.speaker.bio.$invalid ? 'md-invalid' : ''">
              <label>{{ $t('SPEAKER.BIO') }}</label>
              <md-textarea :disabled="isSaving" v-model.trim="speaker.bio">
              </md-textarea>
              <span class="md-error" v-if="!$v.speaker.bio.required">
                {{ $t('SPEAKERS_EDIT.ERRORS.REQUIRED') }}
              </span>
            </md-field>
            <div class="md-layout md-gutter">
              <div class="md-layout-item md-small-size-100">
                <div class="field-img">
                  <div class="img">
                    <img alt="" src="@/assets/logo-github.svg" />
                  </div>
                  <div class="field">
                    <md-field>
              ...
              <div class="md-layout-item md-small-size-100">
                <div class="field-img">
                  <div class="img">
                    <img alt="" src="@/assets/logo-twitter.svg" />
                  </div>
                  <div class="field">
                    <md-field>
          ...
          <md-card-actions>
            ...
            <md-button
              :disabled="$v.speaker.$invalid || isSaving"
...
export default {
  ...
  methods: {
    findAllSpeakersFromConferenceHall() {
      ...
      SpeakersService.findAllFromConferenceHall(this.$route.params.editionId)
        .then(res => {
          ...
          this.availableSpeakers = this.speakers.map(
    ...
    initSpeakerFromConferenceHall(speakerName) {
      const speakerFound = this.speakers.find(
        speaker => speaker.displayName === speakerName
      );
    ...
    save() {
      ...
      if (this.isUpdatingMode) {
        promise = SpeakersService.update(
          this.$route.params.speakerId,
          this.speaker
        );
      } else {

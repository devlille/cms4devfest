<template>
  <div class="editions-dashboard mw-basic-layout">
    <div class="mw-content">
      ...
      <md-tabs class="md-elevation-2" md-alignment="fixed" v-if="!isLoading">
        <md-tab
          :md-disabled="Object.keys(partners).length === 0"
          :md-label="
            $tc(
              'EDITIONS_DASHBOARD.PARTNERS.LABEL',
          ...
          >
            ...
            >
              ...
              <div class="md-list-item-text">
                ...
                <span>
                  <a :href="`https://${partner.url}`" target="_blank">
                    {{ partner.url }}
                  </a>
                  &nbsp;-&nbsp;
                  {{ partner.level }}
                ...
                <span
                  >{{ $t('PARTNER.ACTIVE_ON') }} :
                  {{ partner.activeOn | date('DD/MM/YYYY') }} ({{
                    partner.activeOn | date('v')
                  }})</span
                >
              </div>
        ...
        <md-tab
          ...
          :md-label="
            $tc(
              'EDITIONS_DASHBOARD.SPEAKERS.LABEL',
        ...
        <md-tab
          :md-disabled="Object.keys(talks).length === 0"
          :md-label="
            $tc('EDITIONS_DASHBOARD.TALKS.LABEL', Object.keys(talks).length, [
              Object.keys(talks).length
            ])
          "
          id="tab-talks"
          md-icon="event"
        >
          <md-list v-if="Object.keys(talks).length > 0">
            <md-list-item :key="`talk_${id}`" v-for="(talk, id) in talks">
              ...
              <md-menu>
                ...
                <md-menu-content>
                  <md-menu-item
                    :to="{
                      name: 'talks-edit',
                      params: { editionId: $route.params.editionId, talkId: id }
                    }"
                  >
                    {{ $t('ACTIONS.MODIFY') }}
                  </md-menu-item>
    ...
    <md-speed-dial class="cms-edit-btn">
      ...
      <md-speed-dial-content>
        <md-button @click="add('editions-import')" class="md-icon-button">
          <md-icon>save</md-icon>
        </md-button>
        <md-button @click="add('talks-edit')" class="md-icon-button">
          <md-icon>event</md-icon>
        </md-button>
        <md-button @click="add('speakers-edit')" class="md-icon-button">
          <md-icon>record_voice_over</md-icon>
        </md-button>
        <md-button @click="add('partners-edit')" class="md-icon-button">
          <md-icon>business_center</md-icon>
          <md-tooltip md-direction="left"
            >{{ $t('EDITIONS_DASHBOARD.PARTNERS.CREATE') }}
          </md-tooltip>
        </md-button>
      </md-speed-dial-content>
    </md-speed-dial>
